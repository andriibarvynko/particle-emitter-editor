...
interface Props {
  list: ValueList<number>;
  onChange: (list: ValueList<number>) => void;
  valueLabel?: string;
  min?: number;
  max?: number;
  step?: number;
...
export function ValueListEditor({ list, onChange, valueLabel = 'Value', min, max, step = 0.01 }: Props) {
  const steps = list.list;

  const updateStep = (index: number, patch: Partial<ValueStep<number>>) => {
    const next = steps.map((s, i) => (i === index ? { ...s, ...patch } : s));
    onChange({ ...list, list: next });
  };

  const addStep = () => {
    // Insert before the last keyframe at midpoint
    const len = steps.length;
    if (len < 2) {
      onChange({ ...list, list: [...steps, { value: 0, time: 1 }] });
      return;
    }
    const prev = steps[len - 2]!;
    const last = steps[len - 1]!;
    const midTime = +(((prev.time + last.time) / 2).toFixed(3));
    const midValue = +((prev.value + last.value) / 2).toFixed(3);
    const next = [...steps.slice(0, len - 1), { value: midValue, time: midTime }, last];
    onChange({ ...list, list: next });
  };

  const removeStep = (index: number) => {
    if (steps.length <= 2) return; // keep at least 2
  ...
  return (
    <div className="value-list-editor">
      <div className="value-list-header">
        ...
        <span className="vle-col-value">{valueLabel}</span>
      ...
      <label className="vle-stepped">
        <input
          type="checkbox"
          checked={list.isStepped ?? false}
          onChange={(e) => onChange({ ...list, isStepped: e.target.checked || undefined })}
        />
        <span>Stepped</span>
      </label>
    </div>
